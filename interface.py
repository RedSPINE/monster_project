...
class App(Tk):
    ...
    def doPlay(self):
        ...
        if self.k == 0:
            ...
            # print(gen)
            self.print_watch(gen[0])

    def print_watch(self, watch):
        j = 0

        for j in range(0, len(watch)):
            item = watch[j]
            ...
            if item.__class__.__name__ == "Gear":
                ...
                x = item.x_position / 4
                y = item.y_position / 4
                cercle = self.Gui.create_oval(10 + x - 20, 10 + y - 20, 10 + x + 20, 10 + y + 20, fill="",
                                              outline="green",
                                              width=3)
...
class Draw(Tk):
    ...
    def __init__(self):
        Tk.__init__(self)
        self.title("Zone Test -- DANGER")

        self.toolbar = Frame(self)

        self.buttonQuit = Button(self.toolbar, text="Exit", command=self.doQuit)
        self.buttonQuit.pack(side=RIGHT, padx=2, pady=2)
        self.buttonQuit = Button(self.toolbar, text="Clear", command=self.doNew)
        self.buttonQuit.pack(side=LEFT, padx=2, pady=2)
